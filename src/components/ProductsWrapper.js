...
export default function ProductsWrapper () {
	...
	async function getProducts() {
		...
		if (priceParameters !=='') { // check if there are price query parameters
			...
			// empty query if parameters are not available
			priceQuery = ''; 
		}

		try {
			await axios.get('http://eze-backend-api.herokuapp.com/api/iphones/buy?page='+page+'&limit='+limit+''+cleanParams+''+cleanPriceParams)  //String list = Arrays.toString(customers.toArray()).replace("[", "").replace("]", "");
	...
	// Check the response for data and determine what to render
